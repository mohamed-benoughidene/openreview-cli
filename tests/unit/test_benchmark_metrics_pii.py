...
The match predicate is MUC/CoNLL-style partial matching: a detection
``(value, type)`` is correct when the type matches exactly (case-insensitive)
and the values overlap as a substring or via at least one shared non-trivial
token (tokens split on whitespace and non-alphanumerics).
"""
...
def test_type_mismatch_never_matches() -> None:
    # Both types must be supplied to compare; mismatch → no match even for
    # identical values.
    assert not _values_match(
        "AutoName1 Smith", "autoname1 smith", gt_type="PERSON", det_type="LOCATION"
    )
    assert not _values_match("AutoName1 Smith", "autoname1 smith", gt_type="PERSON", det_type="ORG")


def test_type_compared_case_insensitively() -> None:
    # Same type with different casing counts as a match.
    assert _values_match("AutoName1 Smith", "autoname1 smith", gt_type="PERSON", det_type="person")
    assert _values_match("AutoName1 Smith", "autoname1", gt_type="PERSON", det_type="person")
    assert not _values_match("AutoName1 Smith", "autoname1", gt_type="PERSON", det_type="ORG")
...
def test_false_positive_lowers_precision() -> None:
    """A detection that matches no GT value of its type counts against precision."""
    documents = [
        (
            "doc1",
            "AutoName1 Smith",
