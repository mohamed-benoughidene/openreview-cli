...
async def test_start_review_button_pushes_wizard() -> None:
    """Home tab 'Start review' button pushes ReviewWizard."""
    from openreview_cli.tui.app import OpenReviewApp
    from openreview_cli.tui.screens.review_wizard import ReviewWizard
    ...
    async with app.run_test(size=(120, 40)) as pilot:
        ...
        assert any(isinstance(s, ReviewWizard) for s in app._screen_stack), (
            "ReviewWizard should be open after clicking 'Start review'"
        )
        ...
        with patch("openreview_cli.storage.reviews.save_review_report") as mock_save:
            ...
            try:
                ...
                async with app.run_test(size=(120, 40)) as pilot:
                    ...
                    # Signal handler fires, cancels review, pops screen, exits app
                    await asyncio.sleep(0.5)
            finally:
                signal.signal(signal.SIGTERM, old_sigterm)
                _review_mod._tui_cancel_requested = False
