...
3. Editing the source document after ingest changes its hash. The old index then becomes unreachable from the TUI, which only ever addresses the current hash, and is removable only by hand.
