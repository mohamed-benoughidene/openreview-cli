...
### Amendment 2026-08-19 (R8): scoring & annotation semantics

- Q: How is a detection compared against ground truth? → A: MUC/CoNLL-style partial (token-overlap) matching. A detection `(value, type)` is correct when the type matches exactly (case-insensitive) AND the values overlap: the detection value is a substring of the GT value, the GT value is a substring of the detection value, or the two share at least one non-trivial token (tokens split on whitespace and non-alphanumerics). This is standard NER partial matching (exact-full-string matching cannot score span-level LOCATION/ORGANIZATION/PERSON detections). The predicate is deterministic and pure.
...
- **FR-006**: System MUST implement PII accuracy validation that computes recall (percentage of true PII detected) and precision (percentage of detections that are true PII) against the ground truth corpus, using MUC/CoNLL-style partial (token-overlap) matching as amended 2026-08-19 (R8): a detection matches a ground-truth entity when the type matches exactly (case-insensitive) and the values overlap as a substring (either direction) or share at least one non-trivial token (tokens split on whitespace and non-alphanumerics). A single detection may credit multiple same-type ground-truth entities it covers (e.g. a merged "A Inc. and B LLC" span), and per-type breakdowns count each ground-truth entity independently.
