...
### Amendment 2026-09-24 (R8): scoring & annotation semantics

- Q: How is a detection compared against ground truth? -> A: Span-level (type-agnostic) partial matching. A detection is correct when its value overlaps a GT value, whatever the entity type label: the detection value is a substring of the GT value, the GT value is a substring of the detection value, or the two share at least one non-trivial token (tokens split on whitespace and non-alphanumerics). The type is deliberately not compared. A wrong-type placeholder still redacts the right span, so it covers the PII and is not a false positive; labelling quality is a separate concern, tracked by the per-type recall breakdown rather than the aggregate precision/recall gate. This is standard NER partial matching (exact-full-string matching cannot score span-level LOCATION/ORGANIZATION/PERSON detections). The predicate is deterministic and pure. This adopts the span-level reconciliation recorded at R8 closure (2026-08-19), which the evaluator did not previously reflect.
- Q: What is the nature of the validation corpus? -> A: Synthetic. `tests/fixtures/pii/seeded_contracts/` is generated by `tests/fixtures/pii/generate_seeded.py` from three templated contract forms; accuracy numbers reflect detection on templated documents, not real-world contracts.
...
- **FR-005**: System MUST populate the PII accuracy validation corpus (`tests/fixtures/pii/seeded_contracts/ground_truth.json`) with at least 50 contracts containing annotated PII entities (party names, dates, amounts, addresses). The corpus is synthetic — regenerated via `tests/fixtures/pii/generate_seeded.py` (never hand-edited) — and every annotated entity MUST appear verbatim in its document text.
- **FR-006**: System MUST implement PII accuracy validation that computes recall (percentage of true PII detected) and precision (percentage of detections that are true PII) against the ground truth corpus, using span-level (type-agnostic) partial (token-overlap) matching as amended 2026-09-24 (R8): a detection matches a ground-truth entity when the values overlap as a substring (either direction) or share at least one non-trivial token (tokens split on whitespace and non-alphanumerics), regardless of the entity type label. A wrong-type placeholder still redacts the right span, so it counts as covering the PII; labelling quality is tracked separately by the per-type breakdown. A single detection may credit multiple ground-truth entities it covers (e.g. a merged "A Inc. and B LLC" span), and per-type breakdowns count each ground-truth entity independently.
...
- The validation corpus (`tests/fixtures/pii/`) will be populated with synthetic contracts containing known PII entities. The corpus does not need to be representative of real-world contracts for initial validation. It is generated by `tests/fixtures/pii/generate_seeded.py` and regenerated only via that script.
