...
- [X] T034 Update `gateway models` CLI command to call `ModelRegistry.discover_ollama()` for Ollama provider and merge dynamic results into displayed model list per US6/AC2 (done — `gateway models <provider>` merges discovered Ollama models by default, with a `--no-discover` opt-out that skips the localhost probe)
...
- [x] T039 [US4] Add the optional per-slot backup-model surface per FR-016 / US4-AC4: the `gateway fallback <slot> <model>|--clear` CLI subcommand (`app.py`) and the optional backup-model prompt in the CLI setup wizard (`gateway/wizard.py`). Primary-only slots (`embedding`/`reranking`) are rejected up-front. The Textual gateway wizard/settings surface is tracked separately.
